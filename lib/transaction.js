...
var samuraiRecipes = require('./samurai').recipes;
...
/**
 * ## MAPPINGS
 * *Mappings between Samurai fields and Samurai node names*
 */
...
transaction.recipes = {
  ...
  paymentMethod: samuraiRecipes.paymentMethod
};
...
function getTransactionPath(type, transactionId) {
  ...
  return subpath + (transactionId || config.option('processorToken')) + '/' + 
    type + '.xml';
}
...
transaction.Transaction = function(opts) {
  // opts are required
  if (!opts) {
    throw new SamuraiError('system', 'Missing options object', null);
  }
  // opts.type are required
  if (!opts.type || VALID_TRANSACTIONS.indexOf(opts.type) < 0) {
    throw new SamuraiError('system', 'Missing or invalid transaction type', null);
  }
  // opts.data are required
  if (!opts.data) {
    throw new SamuraiError('system', 'Missing payload data');
  }
 ...
 * is an undiscovered flaw in Samurai itself).
 * 
 ...
 *
 * @param {samurai.Card} [card] Payment method to use for the transaction
...
transaction.Transaction.prototype.process = function(card, callback) {
  ...
  if (self.useToken && (!card || !card.token)) {
    callback(new SamuraiError('system', 'Card has no token', null));
    return;
  }
