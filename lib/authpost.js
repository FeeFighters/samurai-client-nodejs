...
var SAMURAI_VERSION = require('./config').SAMURAI_VERSION;
...
function generateCreds() {
  var credential = config.option('merchantKey') + ':' + config.option('merchantPassword');
  return 'Basic ' + new Buffer(credential).toString('base64');
}

/**
 * ## parseMessages(xml)
 * *Parse error messages and return a SamuraiError object*
 * 
 * This function extracts the error block from the Samurai's response XML, and
 * parses the error messages into SamuraiError object with individual messages
 ...
 * @returns {SamuraiError[]} Array of samurai error objects
 * @private
 */
function parseMessages(xml) {
  ...
  return new SamuraiError('gateway', 'Gateway returned error or info messages', messages);
}
...
exports.makeRequest = function(opts, callback) {
  ...
  if (!config.option('merchantKey') || !config.option('merchantPassword')) {
    callback(new SamuraiError('system', 'Samurai is not configured', null));
  }
  ...
  headers = {
    ...
    'User-Agent': 'Samurai/' + SAMURAI_VERSION + ' Node.js/' + process.version,
  ...
  request = https.request(requestOptions, function(response) {
    ...
    response.on('end', function(err) {
      ...
      // System error
      if (err) {
        error = new SamuraiError('system', 'There was an error with the request', err);
      } else {
  ...
  request.on('error', function(err) {
    callback(new SamuraiError('system', 'There was an error with the request', err));
  });
